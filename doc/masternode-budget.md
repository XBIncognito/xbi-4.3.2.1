...
XBI now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<xbi_address\> \<monthly_payment_xbi\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ XBI, _12_ times over the course of a year totaling _24000_ XBI.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<xbi_address\> \<monthly_payment_xbi\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` XBI.
 ...
 * vote-many          - Vote on a XBI initiative
 * vote-alias         - Vote on a XBI initiative
 * vote               - Vote on a XBI initiative/budget
