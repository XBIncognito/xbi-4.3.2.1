...
*Setup instructions for a gitian build of XBI using a Debian VM or physical system.*
...
Clone the git repositories for xbi and gitian and then checkout the xbi version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xbi repository
...
To build XBI (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xbi repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xbi/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/XBI
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/XBI.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit xbi=${COMMIT} --url xbi=${URL} ../XBI/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xbi=${COMMIT} --url xbi=${URL} ../XBI/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xbi=${COMMIT} --url xbi=${URL} ../XBI/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xbi-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xbi-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/xbi-build.assert
```
