                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXBI ought to be enough for anybody." - Bill Gates, 2017
    ui->zXBIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXBISyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXBI->setEnabled(false);
        ui->pushButtonMintzXBI->setToolTip(tr("zXBI is currently disabled due to maintenance."));

        ui->pushButtonSpendzXBI->setEnabled(false);
        ui->pushButtonSpendzXBI->setToolTip(tr("zXBI is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXBIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXBI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXBI is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXBI...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXBI in ") +
...
void PrivacyDialog::on_pushButtonSpendzXBI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXBI is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXBI
        sendzXBI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXBI
    sendzXBI();
}

void PrivacyDialog::on_pushButtonZXBIControl_clicked()
{
    ZXBIControlDialog* zXBIControl = new ZXBIControlDialog(this);
    zXBIControl->setModel(walletModel);
    zXBIControl->exec();
}

void PrivacyDialog::setZXBIControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXBISelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXBI()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid XBI Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXBIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXBI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XBI </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXBI</b>";
    ...
    // use mints from zXBI selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXBIControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXBIControlDialog::GetSelectedMints();
    }
    ...
    // Clear zxbi selector in case it was used
    ZXBIControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXBI Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " XBI, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXBI Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXBI </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXBI "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXBI "));
    ui->labelzXBIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXBISyncStatus->setVisible(fShow);
}
