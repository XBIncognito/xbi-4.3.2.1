...
#include "ui_zxbicontroldialog.h"
...
std::list<std::string> ZXBIControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXBIControlDialog::listMints;

ZXBIControlDialog::ZXBIControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXBIControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXBIControlDialog::~ZXBIControlDialog()
{
    delete ui;
}

void ZXBIControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXBIControlDialog::updateList()
{
...
void ZXBIControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXBIControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXBI_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXBIControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXBIControlDialog::GetSelectedMints()
{
...
void ZXBIControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
